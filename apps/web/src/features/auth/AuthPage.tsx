...
export function AuthPage() {
  ...
  const [mode, setMode] = useState<'login' | 'register'>(token ? 'login' : 'login');
    ...
    <section className="card stack">
      ...
      <form className="stack" onSubmit={submit}>
        <label className="field">
          <span>用户名</span>
          <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="2-64 个字符" />
        </label>
        <label className="field">
          <span>密码</span>
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="至少 6 位" />
        </label>
        <button className="btn primary" disabled={loading} type="submit">{loading ? '提交中...' : mode === 'login' ? '登录' : '注册'}</button>
      </form>
